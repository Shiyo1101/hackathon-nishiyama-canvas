...
import { createUserImageService } from "../user-images/user-images.service";
  ...
  app.post(
    ...
    async (c: Context<{ Variables: AuthenticatedVariables }>) => {
      try {
        ...
        // UserImageをDBに保存
        const userImage = await userImageService.createUserImage({
          userId,
          imageUrl: result.secureUrl,
          thumbnailUrl: result.secureUrl, // サムネイル未実装のため同じURLを使用
